...
def run_training(ModelClassName=conf.MODELCLASSNAME,
                 data_dir=conf.TFREC_DESTINATION, # TODO trennen destination und standardpfad - oder doch nicht?
    ...
    train_data = run_data_pipeline(data_dir, data_dir='train', 
                                AUTOTUNE=AUTOTUNE)
    test_data = run_data_pipeline(data_dir, data_dir='test', 
                                AUTOTUNE=AUTOTUNE)
    noise_data = run_data_pipeline(data_dir, data_dir='noise', 
                                AUTOTUNE=AUTOTUNE)
    ...
    if steps_per_epoch and n_train_set // batch_size < epochs*steps_per_epoch:
        train_data = train_data.repeat(epochs*steps_per_epoch//(n_train_set//batch_size)+1)
...
############## PLOT TRAINING PROGRESS & MODEL EVALUTAIONS ###################
    ...
    create_and_save_figure(ModelClass, data_dir, batch_size, time_start,
...
if __name__ == '__main__':
    ...
    for i in range(len(time_augs)):
        run_training(data_dir=data_dir,
                     epochs=epochs[i],
