...
def annotate_options(key='annot'):
    ...
    if not st.session_state.b1:
        ...
        if preset_option == 1 or preset_option == 0:
            ...
            default_path = st.radio(f"The annotations I would like to filter are located in `{Path(conf['generated_annotations_folder']).resolve()}`:", 
                     ('Yes', 'No'), key='radio_' + key, horizontal=True)
            if default_path == 'Yes':
                config['generated_annotation_source'] = utils.open_folder_dialogue(key='folder_default_' + key,
                ...
                config['generated_annotation_source'] = utils.open_folder_dialogue(path, key='folder_' + key,
                                                                                label="From the timestamps folders, choose the one you would like to work on.",
                                                                                filter_existing_annotations=True)
                if (Path(config['generated_annotation_source']).stem + Path(config['generated_annotation_source']).suffix 
