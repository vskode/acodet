...
def output():
    ...
    if st.session_state.run_option == 1:
        if st.session_state.preset_option == 0:
            ...
            disp.show_annotation_predictions()
            disp.create_tabs(
                additional_headings=['Hourly annotations plots',
                                     'Hourly presence plots'])
            disp.show_stats()
            disp.show_individual_files()
...
class InitPlots():
    def __init__(self, disp_obj, tab_number=3) -> None:
        ...
        if not self.plots_paths:
            st.write('No analysis files found for this dataset. '
    ...
    def create_tabs(self):
        self.tabs = {'binary': getattr(self.disp_obj, f'tab{self.tab_number}'),
    ...
    def init_tab(self, tab, key):
        with tab:
            ...
            self.chosen_dataset = (self.disp_obj.annots_path
                                   .joinpath('thresh_0.5')
                                   .joinpath(chosen_dataset)
                                   .joinpath('analysis'))

            
            limit = st.radio('What limit would you like to set?',
...
class PlotPresence():
    ...
    def plot_df(self):
        df = pd.read_csv(self.plot_tabs
                         .chosen_dataset
                         .joinpath(f'{self.path_prefix}_{self.limit}.csv'))
        df.index = pd.DatetimeIndex(df.Date)
        df = df.reindex(pd.date_range(df.index[0], df.index[-1]), fill_value=np.nan)
        
        h_of_day_str = ['%.2i:00' % i for i in range(24)]
        h_pres = df.loc[:, h_of_day_str]
        
        fig = px.imshow(h_pres.T,
                        labels=dict(x="Date", 
                                    y="Time of Day", 
                                    color=self.cbar_label),
                x=df.index,
                y=h_of_day_str,
    ...
    def create_sliders(self):
        thresh = st.slider('Threshold', 0.35, 0.99, conf[self.thresh], 0.01,
                           key=f'thresh_slider_{self.key}')
        
        limit = st.slider('Limit', 1, self.limit_max, conf[self.limit], 1,
