...
def read_annotations_from_file(comb_anno_folder):
    """
    Reads the CSV files generated by this module

    :param comb_anno_folder: a folder containing combined_annotations CSV files.
    :return:
        train_df: dataframe of training examples
        val_df: dataframe of validation examples
        eval_df: dataframe of test examples
    """
    combined_annots = Path(comb_anno_folder) / 'combined_annotations.csv'
    explicit_noise = Path(comb_anno_folder) / 'explicit_noise.csv'
    ...
    trainval_df = df[df.subset != 'test']
    eval_df = df[df.subset == 'test']

    # Split train/val
    if 'val' in trainval_df.subset.values:
        train_df = trainval_df[trainval_df.subset != 'val']
        val_df = trainval_df[trainval_df.subset == 'val']
    else:
        rand_ints = np.random.permutation(len(trainval_df))
        border = int(len(trainval_df) * 0.8)
        train_df = trainval_df.iloc[rand_ints[:border]]
        val_df = trainval_df.iloc[rand_ints[border:]]

    print(f"Data split sizes: (train = {len(train_df)}, val = {len(val_df)}, test = {len(eval_df)})")
