...
# main()
def plot_counts_per_hour(path):
    df = pd.read_csv(Path(path).joinpath('hourly_annotation_counts.csv'))
    h_pres = df.loc[:, h_of_day_str()]
    h_pres.index = df['Date']
    plt.figure()
    plt.title('Annotation counts for each hour')
    sns.heatmap(h_pres.T)
    plt.ylabel('hour of day')
    plt.tight_layout()
    plt.savefig(Path(path).parents.joinpath('hourly_counts_plot.png'))

def plot_hourly_presence(path):
    df = pd.read_csv(Path(path).joinpath('hourly_presence.csv'))
    h_pres = df.loc[:, h_of_day_str()]
    h_pres.index = df['Date']
    plt.figure()
    plt.title('Annotation counts for each hour')
    ...
    plt.ylabel('hour of day')
    plt.tight_layout()
    plt.savefig(Path(path).parents.joinpath('hourly_presence_plot.png'))
...
plot_counts_per_hour('../generated_annotations/2022-11-25_12')